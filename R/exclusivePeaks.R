  ...
  if (!missing(excluded_peak_id) && length(excluded_peak_id) > 0) {
    ...
    for (i in excluded_peak_id) {
      ...
      if (is.null(peak_i)) {
        ...
        message(paste0(
          "... ... peak file loaded successfully for id '", i, "'"
        ))
      }
    }
    message("... Done loading TFBS(s) from TFregulomeR")
    ...
    if (isTFregulome_target) {
      ...
      if (nrow(target_peak_i) > 0) {
        ...
        if (nrow(motif_of_peakTarget_done_exclusive) > 0) {
          # nPeaks
          suppressWarnings(peaks_with_motif_gr <- subsetByOverlaps(
            bed_target_done_exclusive,
          ...
          motif_matrix_of_peakTarget_done_exclusive <- formMatrixFromSeq(
            input_sequence = as.vector(motif_of_peakTarget_done_exclusive_allInfo$sequence),
            motif_format = motif_type
          )
          ...
          if (isMethMotifID_target) {
            ...
            if (nrow(meth_level_target) == 0) {
              beta_score_matrix_of_peakTarget_done_exclusive <- formBetaScoreFromSeq(
                input_meth = data.frame(),
                WGBS_replicate = WGBS_replicate_target,
                motif_len = motif_len_target
              )
            } else {
              colnames(meth_level_target) <- c(
                "chr", "start", "end", "meth_score", "C_num", "T_num",
                "seq_chr", "seq_start", "seq_end", "strand", "weight",
              ...
              suppressWarnings(meth_level_peakTarget_done_exclusive <- unique(
                as.data.frame(
                  subsetByOverlaps(
                    meth_level_target_grange,
              ...
              beta_score_matrix_of_peakTarget_done_exclusive <- formBetaScoreFromSeq(
                input_meth = meth_level_peakTarget_done_exclusive_allInfo,
                WGBS_replicate = WGBS_replicate_target,
                motif_len = motif_len_target
              )
            }
          ...
          MethMotif_target@MMmotif <- updateMMmotif(
            MethMotif_target@MMmotif,
            motif_format = motif_type,
